...
def _collect_episode(
    *,
    ...
    history_layout = model.layout if model is not None else _arx_layout(profile)
    output_history = np.vstack([initial_output] * history_layout.lag_count)
    control_history = np.vstack(
        [[last_setpoints[zone] for zone in zones]] * history_layout.completed_history_steps
    )
    times: list[int] = []
    ...
    for step in range(steps):
        ...
        if role in {"fit", "holdout"}:
            ...
            forecast_window = build_mpc_forecast_window(
                profile,
                forecast,
                zones,
                step=step,
                action_time=action_time,
                horizon_steps=hierarchical.model.layout.horizon_steps,
                step_seconds=STEP_SECONDS,
            )
...
def _verify_mpc_model_directory(case: str, target: Path) -> dict[str, Any]:
    ...
    degraded_admission = admission_mode in {
        "post_result_method_degraded",
        "preregistered_method_degraded",
    }
    preregistered = schema_version == 3
    fresh_peak = (
    ...
    lifecycle_valid = (
        ...
        and (
            degraded_admission
            or fresh_validation.get("fallback_count") == 0
            and (not preregistered or fresh_validation.get("comfort_target_met") is True)
        )
        and fresh_comfort_target_valid
        and isinstance(card.get("validation"), dict)
        and card["validation"].get("eligible")
        is (
            int(fresh_validation["fallback_count"]) == 0
            and (not preregistered or fresh_validation.get("comfort_target_met") is True)
        )
        ...
        and (
            not preregistered
            or isinstance(card.get("release_path"), str)
            and card.get("release_path") == manifest.get("release_path")
        )
    )
    model_structure = card.get("model_structure")
    layout_contract = bool(
        model.layout.zones == zones
        and model.layout.horizon_steps == 4
        and (
            schema_version in {1, 2}
            and model_structure == "vector_arx_four_lag_four_step"
            and model.layout.lag_count == 4
            or model_structure == "vector_arx_current_plus_four_history_four_step"
            and model.layout.lag_count == 5
...
def verify_frozen_mpc_model(case: str, target: Path | None = None) -> dict[str, Any]:
    root = repository_root().resolve()
    suite_target = target or root / "models" / "mpc"
    if not suite_target.is_absolute():
        suite_target = root / suite_target
    suite_target = suite_target.resolve()
    try:
    ...
    if manifest.get("schema_version") == 3:
        verified["mpc_release_path"] = suite_target.relative_to(root).as_posix()
    return verified
