...
class ArxLayout:
    ...
    @property
    def completed_history_steps(self) -> int:
        return self.lag_count - 1
...
class FittedArxModel:
    ...
    def rollout(
        ...
        controls = _completed_control_history(self.layout, control_history)
        predictions: list[NDArray[np.float64]] = []
        negative_power_count = 0
        for horizon in range(self.layout.horizon_steps):
            control_terms = np.vstack((future_controls[horizon], controls))
            predicted = self.predict_next(outputs, control_terms, disturbances[horizon])
            if predicted[-1] < 0:
...
def _completed_control_history(
    layout: ArxLayout, control_history: NDArray[np.float64]
) -> NDArray[np.float64]:
    controls = np.asarray(control_history, dtype=np.float64)
    expected = (layout.completed_history_steps, layout.control_dimension)
    if controls.shape != expected or np.any(~np.isfinite(controls)):
        raise ValueError("ARX completed control history has the wrong shape or values")
